...
class _CompletedJobState extends State<CompletedJob> {
  ...
  void _get_job_data_Completed() async {
    var newjob_data = await controller.fetch_job_data_Completed();
    setState(() {
                ...
                return ListTile(
                    onTap: () {
                      ...
                      Navigator.push(context,
                          MaterialPageRoute(builder: (context) => JobData_Assign()));
                    },
                    title: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('${job_datas_completed[index].hospital}',
                         style: TextStyle(
                                fontWeight: FontWeight.bold,
                              )),
                        Text('${job_datas_completed[index].device_name} : ${job_datas_completed[index].sn}'),
                        Text('date : ${job_datas_completed[index].date.substring(0,10)}'),
                        Text(
                            'error code ${job_datas_completed[index].error_code}'),
                      ],
                    ),
                    trailing: IconButton(
                      onPressed: () {
